...
function getPacksDirectory(): string {
  ...
  // Packs ship at the plugin package root (clarityburst-plugin/ontology-packs).
  // From source layout this is ../ontology-packs (src/ → root); from compiled
  // output it is ../../ontology-packs (dist/src/ → root). Try both.
  const candidates = [
    path.resolve(__dirname, "../ontology-packs"),
    path.resolve(__dirname, "../../ontology-packs"),
  ];
